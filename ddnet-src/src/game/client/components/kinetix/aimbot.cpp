...
// =========================================================
// ZenithTee v1.1: persistent weapon settings
// Weapon configs live in component memory (not config variables) and are
// serialized to "zenithtee_weapons.cfg": one line per field, format
// "ab|tb <weapon> <field> <value>". Loaded at OnInit, saved debounced on
...
{
	LoadSettings();
	m_SavedSettingsHash = ComputeSettingsHash();
	m_LastSaveTime = time_get();
}

void CAimBot::OnShutdown()
{
	SaveSettings();
}

uint64_t CAimBot::ComputeSettingsHash() const
{
	// FNV-1a over all fields of both weapon arrays, field by field
	// (padding-safe, unlike a raw byte hash).
	uint64_t h = 1469598103934665603ull;
	auto Mix = [&h](uint64_t V) {
		h ^= V;
		h *= 1099511628211ull;
	};
	auto MixStr = [&](const char *pStr) {
		for(const char *p = pStr ? pStr : ""; *p; p++)
			Mix((uint64_t)(uint8_t)*p);
	};
	for(int w = 0; w < NUM_WEAPONS_AIM; w++)
	{
		const SWeaponSettings &s = m_AimBotWeapons[w];
		Mix(s.m_Enabled ? 1 : 0);
		Mix((uint64_t)s.m_AimMode);
		Mix((uint64_t)(s.m_Silent ? 1 : 0));
		Mix((uint64_t)(s.m_Smooth * 1000.0f));
		Mix((uint64_t)(s.m_Fov * 10.0f));
		Mix((uint64_t)s.m_Radius);
		Mix(s.m_UseRaycastShow ? 1 : 0);
		Mix((uint64_t)s.m_Rules);
		Mix(s.m_UseAngle ? 1 : 0);
		Mix(s.m_Predict ? 1 : 0);
		Mix((uint64_t)s.m_TeamFilter);
		Mix((uint64_t)s.m_FriendFilter);
		Mix((uint64_t)s.m_DummyFilter);
		Mix((uint64_t)s.m_ClanmateFilter);
		Mix((uint64_t)s.m_TargetsFilter);
		Mix((uint64_t)s.m_FreezeFilter);
		Mix((uint64_t)s.m_Priority);
		MixStr(s.m_aTargetsFilter);
	}
	for(int w = 0; w < NUM_WEAPONS_TRIGGER; w++)
	{
		const SWeaponSettings &s = m_TriggerBotWeapons[w];
		Mix(s.m_Enabled ? 1 : 0);
		Mix((uint64_t)s.m_Trigger);
		Mix((uint64_t)s.m_TriggerMode);
		Mix((uint64_t)(s.m_Fov * 1000.0f));
		Mix((uint64_t)s.m_Radius);
		Mix(s.m_UseRaycastShow ? 1 : 0);
		Mix((uint64_t)s.m_Rules);
		Mix((uint64_t)s.m_TeamFilter);
		...
		Mix((uint64_t)s.m_FreezeFilter);
		Mix((uint64_t)s.m_Priority);
		Mix((uint64_t)s.m_RandomLatency);
	}
	return h;
}

void CAimBot::WriteWeaponSettings(IOHANDLE File, const char *pSection, const SWeaponSettings &Settings, int Weapon) const
{
	char aLine[256];
	auto W = [&](const char *pField, const char *pValue) {
		str_format(aLine, sizeof(aLine), "%s %d %s %s\n", pSection, Weapon, pField, pValue);
		io_write(File, aLine, str_length(aLine));
	};
	auto WInt = [&](const char *pField, int V) {
		char aVal[16];
		str_format(aVal, sizeof(aVal), "%d", V);
		W(pField, aVal);
	};
	auto WFloat = [&](const char *pField, float V) {
		char aVal[32];
		str_format(aVal, sizeof(aVal), "%.3f", V);
		W(pField, aVal);
	};
	WInt("enabled", Settings.m_Enabled ? 1 : 0);
	WInt("aim_mode", Settings.m_AimMode);
	WInt("silent", Settings.m_Silent ? 1 : 0);
	WFloat("smooth", Settings.m_Smooth);
	WFloat("fov", Settings.m_Fov);
	WInt("radius", (int)Settings.m_Radius);
	WInt("use_raycast_show", Settings.m_UseRaycastShow ? 1 : 0);
	WInt("rules", Settings.m_Rules);
	WInt("use_angle", Settings.m_UseAngle ? 1 : 0);
	WInt("predict", Settings.m_Predict ? 1 : 0);
	WInt("team", Settings.m_TeamFilter);
	WInt("friend", Settings.m_FriendFilter);
	WInt("dummy", Settings.m_DummyFilter);
	WInt("clanmate", Settings.m_ClanmateFilter);
	WInt("targets_filter", Settings.m_TargetsFilter);
	WInt("freeze", Settings.m_FreezeFilter);
	WInt("priority", Settings.m_Priority);
	WInt("random_latency", Settings.m_RandomLatency);
	WInt("trigger", Settings.m_Trigger);
	WInt("trigger_mode", Settings.m_TriggerMode);
	str_format(aLine, sizeof(aLine), "%s %d targets %s\n", pSection, Weapon,
		Settings.m_aTargetsFilter[0] ? Settings.m_aTargetsFilter : "-");
	io_write(File, aLine, str_length(aLine));
}

void CAimBot::SaveSettings()
{
	IOHANDLE File = Storage()->OpenFile(SETTINGS_FILENAME, IOFLAG_WRITE, IStorage::TYPE_SAVE);
	if(!File)
		return;
	for(int w = 0; w < NUM_WEAPONS_AIM; w++)
		WriteWeaponSettings(File, "ab", m_AimBotWeapons[w], w);
	for(int w = 0; w < NUM_WEAPONS_TRIGGER; w++)
		WriteWeaponSettings(File, "tb", m_TriggerBotWeapons[w], w);
	io_close(File);
	m_SavedSettingsHash = ComputeSettingsHash();
	m_LastSaveTime = time_get();
}

void CAimBot::LoadSettings()
{
	IOHANDLE File = Storage()->OpenFile(SETTINGS_FILENAME, IOFLAG_READ, IStorage::TYPE_SAVE | IStorage::TYPE_ALL);
	if(!File)
		return;
	char *pContents = io_read_all_str(File);
	io_close(File);
	if(!pContents)
		return;

	auto Apply = [&](SWeaponSettings &s, const char *pField, const char *pVal) {
		if(str_comp(pField, "enabled") == 0) s.m_Enabled = atoi(pVal) != 0;
		else if(str_comp(pField, "aim_mode") == 0) s.m_AimMode = atoi(pVal);
		else if(str_comp(pField, "silent") == 0) s.m_Silent = atoi(pVal) != 0;
		else if(str_comp(pField, "smooth") == 0) s.m_Smooth = (float)atof(pVal);
		else if(str_comp(pField, "fov") == 0) s.m_Fov = (float)atof(pVal);
		else if(str_comp(pField, "radius") == 0) s.m_Radius = (float)atof(pVal);
		else if(str_comp(pField, "use_raycast_show") == 0) s.m_UseRaycastShow = atoi(pVal) != 0;
		else if(str_comp(pField, "rules") == 0) s.m_Rules = atoi(pVal);
		else if(str_comp(pField, "use_angle") == 0) s.m_UseAngle = atoi(pVal) != 0;
		else if(str_comp(pField, "predict") == 0) s.m_Predict = atoi(pVal) != 0;
		else if(str_comp(pField, "team") == 0) s.m_TeamFilter = atoi(pVal);
		else if(str_comp(pField, "friend") == 0) s.m_FriendFilter = atoi(pVal);
		else if(str_comp(pField, "dummy") == 0) s.m_DummyFilter = atoi(pVal);
		else if(str_comp(pField, "clanmate") == 0) s.m_ClanmateFilter = atoi(pVal);
		else if(str_comp(pField, "targets_filter") == 0) s.m_TargetsFilter = atoi(pVal);
		else if(str_comp(pField, "freeze") == 0) s.m_FreezeFilter = atoi(pVal);
		else if(str_comp(pField, "priority") == 0) s.m_Priority = atoi(pVal);
		else if(str_comp(pField, "random_latency") == 0) s.m_RandomLatency = atoi(pVal);
		else if(str_comp(pField, "trigger") == 0) s.m_Trigger = atoi(pVal);
		else if(str_comp(pField, "trigger_mode") == 0) s.m_TriggerMode = atoi(pVal);
		else if(str_comp(pField, "targets") == 0)
		{
			if(str_comp(pVal, "-") == 0)
				s.m_aTargetsFilter[0] = '\0';
			else
				str_copy(s.m_aTargetsFilter, pVal, sizeof(s.m_aTargetsFilter));
		}
	};

	for(char *pLine = pContents, *pNext; pLine; pLine = pNext)
	{
		pNext = strchr(pLine, '\n');
		if(pNext)
			*pNext++ = '\0';
		char aKind[16], aField[40], aVal[64];
		if(sscanf(pLine, "%15s %*d %31s %63s", aKind, aField, aVal) < 3)
			continue;
		int idx = 0;
		sscanf(pLine, "%*s %d %*s %*s", &idx);
		if(str_comp(aKind, "ab") == 0 && idx >= 0 && idx < NUM_WEAPONS_AIM)
			Apply(m_AimBotWeapons[idx], aField, aVal);
		else if(str_comp(aKind, "tb") == 0 && idx >= 0 && idx < NUM_WEAPONS_TRIGGER)
			Apply(m_TriggerBotWeapons[idx], aField, aVal);
	}
	free(pContents);
	m_SavedSettingsHash = ComputeSettingsHash();
	m_LastSaveTime = time_get();
...
{
        // ZenithTee v1.1: debounced settings persistence — save when the
        // settings hash changes (catches ALL mutation paths: ClickGUI, Lua
        // code_exec, console) at most once per 0.5s.
        {
                const uint64_t hash = ComputeSettingsHash();
                const int64_t now = time_get();
                if(hash != m_SavedSettingsHash && (now - m_LastSaveTime) > time_freq() / 2)
                {
                        SaveSettings();
                        m_LastSaveTime = time_get();
                }
        ...
        if(g_Config.m_KxAimBot)
        {
                ...
                if(m_AimBotWeapons[6].m_Enabled && (hookHeld || m_AimBotWeapons[6].m_AimMode >= AIMMODE_ALWAYS))
                {
                        if(m_AimBotShowFov)
...
void CAimBot::ApplyAim(int ClientId, const vec2 &MyPos, const SWeaponSettings &Settings, const vec2 &AimPoint)
{
        ...
        else
        {
                ...
                if(Settings.m_AimMode == AIMMODE_SMOOTH)
                {
                        ...
                        pGame->m_Controls.m_aMousePos[g_Config.m_ClDummy] = mix(cur, aimOffset, f);
                }
                else
                {
...
void CAimBot::UpdateAimBot()
{
        ...
        // ZenithTee v1.1: Edge (0) / Trigger (1) trigger on rising fire edge; Always (2) / Smooth (3) every tick.
        // Hook (6) has no reload cadence, so Edge/Trigger use hook edge on m_Hook.
        bool willFire = false;
        bool needWillFire = weapon >= 0 && weapon < NUM_WEAPONS_AIM - 1
                && m_AimBotWeapons[weapon].m_Enabled
                && (m_AimBotWeapons[weapon].m_AimMode == AIMMODE_EDGE || m_AimBotWeapons[weapon].m_AimMode == AIMMODE_TRIGGER);
        if(needWillFire)
        {
        ...
        if(weapon >= 0 && weapon < NUM_WEAPONS_AIM - 1 && m_AimBotWeapons[weapon].m_Enabled)
        {
                ...
                // Edge(0)/Trigger(1) trigger on fire; Always(2)/Smooth(3) every tick.
                bool trigger = (s.m_AimMode >= AIMMODE_ALWAYS) ? true : willFire;
                if(trigger)
                {
        ...
        // Hook (6): Edge/Trigger use hook edge; Always/Smooth = every tick.
        if(m_AimBotWeapons[6].m_Enabled)
        {
                const SWeaponSettings &s = m_AimBotWeapons[6];
                bool trigger = (s.m_AimMode >= AIMMODE_ALWAYS) ? true : (hook && !m_PrevHook);
                if(trigger)
                {
