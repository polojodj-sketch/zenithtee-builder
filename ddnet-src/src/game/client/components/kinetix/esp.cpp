...
// ZenithTee v1.1: emit a solid segment honoring the Line layer setting.
void CEsp::EmitSolidSegment(const vec2 &p0, const vec2 &p1, ColorRGBA col, float halfWidth) const
{
	if(g_Config.m_KxLineRenderingLayer > 0)
		KinetixEnqueueLine(p0, p1, col, halfWidth);
	else
		DrawSegment(p0, p1, col, halfWidth);
}

// ZenithTee v1.1: rectangle outline (4 corners) around a player — ESP box / hitbox.
void CEsp::DrawBoxOutline(const vec2 &Center, float HalfW, float HalfH, int playerIndex) const
{
	int lineSize = KxLineSize(KX_LINE_ESP);
	float halfWidth = lineSize > 0 ? 0.5f + (float)(lineSize - 1) * 0.25f : 0.0f;

	// Per-edge hue offset (0..3) keeps box edges tinted like the target line.
	ColorRGBA c = ColorRGBA(KxLineColorAt(KX_LINE_ESP, playerIndex), true);
	...
	EmitSolidSegment(vec2(tl.x, tl.y), vec2(tr.x, tr.y), c, halfWidth);
	EmitSolidSegment(vec2(tr.x, tr.y), vec2(br.x, br.y), c, halfWidth);
	EmitSolidSegment(vec2(br.x, br.y), vec2(bl.x, bl.y), c, halfWidth);
	EmitSolidSegment(vec2(bl.x, bl.y), vec2(tl.x, tl.y), c, halfWidth);
}
...
void CEsp::OnRender()
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		{
			// Visual box: 64x64 around the rendered body center.
			DrawBoxOutline(target, 32.0f, 32.0f, i);
		}
		if(g_Config.m_KxEspHitboxes)
		{
			// Physical hitbox: the collision box of a tee is 28x28
			// centered at the core position.
			DrawBoxOutline(target, 14.0f, 14.0f, i + MAX_CLIENTS);
		}
	}
}
