...
void CBotNet::ProcessDummy(int Dummy)
{
        ...
        if(g_Config.m_KxAttack)
        {
                ...
                if(TargetID != -1)
                {
                        ...
                        {
                                vec2 aimOffset(dx, dy);
                                // v1.1: Predict Aim — intercept moving targets by aiming at their future position.
                                if(g_Config.m_KxBlockPredict)
                                {
                                        const vec2 targetVel = pGame->m_aClients[TargetID].m_Predicted.m_Vel;
                                        float speed = length(targetVel);
                                        if(speed > 0.1f)
                                        {
                                                // DDNet pistol projectile ~22px/tick (1100 px/s at 50 tick/s).
                                                // Hammer is melee so prediction gives minimal benefit.
                                                float projSpeed = 22.0f;
                                                float t = Dist / projSpeed; // intercept time in ticks
                                                float factor = g_Config.m_KxBlockPredictFactor / 100.0f;
                                                aimOffset.x = dx + targetVel.x * t * factor;
                                                aimOffset.y = dy + targetVel.y * t * factor;
                                        }
                                }
                                SetMousePos(pGame, Dummy, aimOffset);
                        }
