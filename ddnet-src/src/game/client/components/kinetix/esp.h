...
private:
	...
	// ZenithTee v1.1: rectangle outline around a player (ESP box / hitbox).
	// Uses the deferred KinetixEnqueueLine path when Line layer > 0, direct
	// segments otherwise — same policy as EmitLine.
	void DrawBoxOutline(const vec2 &Center, float HalfW, float HalfH, int playerIndex) const;

	// ZenithTee v1.1: emit a solid segment honoring the Line layer setting.
	void EmitSolidSegment(const vec2 &p0, const vec2 &p1, ColorRGBA col, float halfWidth) const;
};
