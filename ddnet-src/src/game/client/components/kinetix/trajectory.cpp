...
// =========================================================
// ZenithTee v1.1: persistent per-type trajectory settings
// Serialized to "zenithtee_trajectory.cfg": one line per field,
// "traj <type> <field> <value>". Loaded at OnInit, saved debounced on
// change (hash diff in OnRender) and on shutdown.
// =========================================================

void CTrajectory::OnInit()
{
	LoadSettings();
}

void CTrajectory::OnShutdown()
{
	SaveSettings();
}

void CTrajectory::SaveSettings()
{
	IOHANDLE File = Storage()->OpenFile(SETTINGS_FILENAME, IOFLAG_WRITE, IStorage::TYPE_SAVE);
	if(!File)
		return;
	char aLine[160];
	for(int t = 0; t < NUM_TRAJ_TYPES; t++)
	{
		const STypeSettings &s = m_aTypes[t];
		auto W = [&](const char *pField, int V) {
			str_format(aLine, sizeof(aLine), "traj %d %s %d\n", t, pField, V);
			io_write(File, aLine, str_length(aLine));
		};
		W("show", s.m_Show ? 1 : 0);
		W("ticks", s.m_PredictionTicks);
		W("gradient", s.m_AlphaGradient ? 1 : 0);
		W("simulate_players", s.m_SimulatePlayers ? 1 : 0);
		W("for_others", s.m_ShowForOtherPlayers ? 1 : 0);
		W("for_current", s.m_ShowForCurrent ? 1 : 0);
	}
	io_close(File);
...
void CTrajectory::LoadSettings()
{
	...
	for(char *pLine = pContents, *pNext; pLine; pLine = pNext)
	{
		...
		if(pNext)
			*pNext++ = '\0';
		char aKind[16], aField[40];
		int type = 0, val = 0;
		if(sscanf(pLine, "%15s %d %31s %d", aKind, &type, aField, &val) != 4)
			continue;
		if(str_comp(aKind, "traj") != 0 || type < 0 || type >= NUM_TRAJ_TYPES)
			continue;
		STypeSettings &s = m_aTypes[type];
		if(str_comp(aField, "show") == 0) s.m_Show = val != 0;
		else if(str_comp(aField, "ticks") == 0) s.m_PredictionTicks = val;
		else if(str_comp(aField, "gradient") == 0) s.m_AlphaGradient = val != 0;
		else if(str_comp(aField, "simulate_players") == 0) s.m_SimulatePlayers = val != 0;
		else if(str_comp(aField, "for_others") == 0) s.m_ShowForOtherPlayers = val != 0;
		else if(str_comp(aField, "for_current") == 0) s.m_ShowForCurrent = val != 0;
	}
	free(pContents);
	m_SavedSettingsHash = ComputeSettingsHash();
	m_LastSaveTime = time_get();
...
uint64_t CTrajectory::ComputeSettingsHash() const
{
	...
	for(int t = 0; t < NUM_TRAJ_TYPES; t++)
	{
		const STypeSettings &s = m_aTypes[t];
		Mix(s.m_Show ? 1 : 0);
		Mix((uint64_t)s.m_PredictionTicks);
		Mix(s.m_AlphaGradient ? 1 : 0);
		Mix(s.m_SimulatePlayers ? 1 : 0);
		Mix(s.m_ShowForOtherPlayers ? 1 : 0);
		Mix(s.m_ShowForCurrent ? 1 : 0);
	}
	return h;
}
...
{
	// ZenithTee v1.1: debounced settings persistence (before the master
	// toggle early-return so changes are caught even while disabled).
	{
