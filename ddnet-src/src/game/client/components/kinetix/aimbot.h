...
public:
        ...
        // ZenithTee v1.1: persistent per-weapon settings. Weapon configs are NOT
        // config variables — they live in this component and are set directly from
        // ClickGUI. They are loaded from "zenithtee_weapons.cfg" at init, saved
        // (debounced ~0.5s) whenever the settings hash changes, and on shutdown.
        void LoadSettings();
        void SaveSettings();
        static constexpr const char *SETTINGS_FILENAME = "zenithtee_weapons.cfg";
        ...
        struct SWeaponSettings
        {
                ...
                // ZenithTee v1.1: separate Silent toggle + new mode set.
                bool m_Silent = true; // AB: silent aim — override server aim without moving the visible crosshair
                int m_AimMode = 0; // 0=edge 1=trigger 2=always 3=smooth (AimBot only)
                float m_Smooth = 0.5f; // AB Smooth mode: interpolation factor per tick (0.05..1.0)
        ...
        // AimBot mode ids (SWeaponSettings::m_AimMode).
        static constexpr int AIMMODE_EDGE = 0; // aim on the tick the weapon fires
        static constexpr int AIMMODE_TRIGGER = 1; // continuous aim + auto-fire while target in zone
        static constexpr int AIMMODE_ALWAYS = 2; // aim every tick, firing left to the player
        static constexpr int AIMMODE_SMOOTH = 3; // like Always, crosshair moves gradually (m_Smooth)
...
private:
        ...
        // ZenithTee v1.1: settings persistence state.
        uint64_t m_SavedSettingsHash = 0;
        int64_t m_LastSaveTime = 0;
        uint64_t ComputeSettingsHash() const;
        // Serializes one weapon's settings as "ab/tb <weapon> <field> <value>" lines.
        void WriteWeaponSettings(IOHANDLE File, const char *pSection, const SWeaponSettings &Settings, int Weapon) const;
