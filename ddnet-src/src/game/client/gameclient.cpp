...
void CGameClient::OnConsoleInit()
{
        ...
        m_vpAll.insert(m_vpAll.end(), {&m_Skins,
                                              ...
                                              &m_ImportantAlert,
                                              &m_DebugHud,
                                              &m_ZenithHud, // ZenithTee v1.1: Spectator list / Array list / Admin Alarm overlay
                                              &m_TouchControls,
                                              &m_Scoreboard,
                                              &m_Statboard,
                                              &m_Motd,
        ...
        // go through all the items in the snapshot and gather the info we want
        {
                ...
                for(int i = 0; i < Num; i++)
                {
                        ...
                        else if(Item.m_Type == NETOBJTYPE_DDNETPLAYER)
                        {
                                ...
                                {
                                        // ZenithTee v1.1: Admin Alarm — detect a player
                                        // transitioning to server auth (joined authed or
                                        // authed mid-game) and raise a chat + sound alert.
                                        if(g_Config.m_KxAdminAlarm &&
                                                m_aClients[Item.m_Id].m_AuthLevel == AUTHED_NO &&
                                                pInfo->m_AuthLevel != AUTHED_NO &&
                                                m_aClients[Item.m_Id].m_Active)
                                        {
                                                const char *pRole = pInfo->m_AuthLevel == AUTHED_ADMIN ? "Admin" :
                                                                                                    pInfo->m_AuthLevel == AUTHED_MOD ? "Moderator" : "Helper";
                                                char aAlarm[256];
                                                str_format(aAlarm, sizeof(aAlarm), "*** ZenithTee Admin Alarm: %s '%s' is now authenticated!",
                                                        pRole, m_aClients[Item.m_Id].m_aName);
                                                m_Chat.AddLine(-1, TEAM_ALL, aAlarm);
                                                m_Sounds.Play(CSounds::CHN_GLOBAL, SOUND_CHAT_HIGHLIGHT, 1.0f);
                                                m_ZenithHud.TriggerAdminAlarm();
                                        }
